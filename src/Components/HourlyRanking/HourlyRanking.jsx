...
import { Clock, CrownIcon, GiftIcon } from 'lucide-react';
...
function top3Leaders(activeFilter, users) {
    const result = [];
    if (activeFilter) {
        let temp = users.sort((a, b) => b.points.hourlyPoint.hourlyPartyPoint - a.points.hourlyPoint.hourlyPartyPoint).slice(0, 3);
        result.push(temp[1]);
        result.push(temp[0]);
        result.push(temp[2]);

    } else {
        let temp = users.sort((a, b) => b.points.hourlyPoint.hourlyLivePoint - a.points.hourlyPoint.hourlyLivePoint).slice(0, 3);
        result.push(temp[1]);
        result.push(temp[0]);
        result.push(temp[2]);
    }
...
function restLeaders(activeFilter, users) {
    ...
    return temp.sort((a, b) => activeFilter===true ? (b.points.hourlyPoint.hourlyPartyPoint - a.points.hourlyPoint.hourlyPartyPoint):(b.points.hourlyPoint.hourlyLivePoint - a.points.hourlyPoint.hourlyLivePoint));
}

function HourlyRanking({ users, primeUser }) {
    ...
    return (
        <div className='relative'>
            ...
            <div className='flex justify-center gap-2 m-2 p-1 bg-gray-200 rounded-full'>
                {/* Daily filter button */}
                <button
                    className={`${activeFilter === true ? "bg-white text-black" : "text-gray-400"} w-full py-1 font-semibold rounded-full`}
                    onClick={() => setActiveFilter(true)}>Hourly Live List</button>
                ...
                <button
                    ...
                    onClick={() => setActiveFilter(false)}>Hourly Party List</button>
            </div>
            </div>

            {users?.length > 0 ?
                // Leader board
                <div className='w-full h-[95vh] bg-gradient-to-b from-[#a37bff] to-[#ee97fe] pt-20 pb-14 overflow-y-auto'>
                    <div className='flex justify-between items-center'>
                        <div />
                        <div />

                        {/* Countdown timer */}
                        <div className='flex gap-x-1 text-white bg-[#9061ff] py-1 px-3 text-[12px] font-semibold text-center rounded-full'>
                            <p>Settlement Time</p>
                            <CountdownTimer />
                        </div>
                    ...
                    <img src="./hourly-ranking.png" alt="Wealth Rank Logo" className='top-0 left-[15%] w-[45%] h-fit m-auto' />
                        ...
                        {top3Leaders(activeFilter, users)?.map((user, index) => (
                            <div key={index} className={`w-full sm:w-[135px] h-full px-1 py-4 justify-center text-center bg-gradient-to-b from-[#f9dcb2] to-[#fdf7ed] ${index === 1 ? "border-[1px] border-x-gray-500 border-t-gray-500" : ""} rounded-t-[10px]`}>
                                ...
                                <p
                                    ...
                                    {activeFilter === true ? user.points.hourlyPoint.hourlyPartyPoint : user.points.hourlyPoint.hourlyLivePoint}</p>
                                {index === 1 &&
            ...
            <div className='absolute bottom-0 w-full flex justify-between p-2 bg-[#a37bff] text-white border-t-[1px] border-gray-700'>
                <div className='flex items-center gap-x-2 text-[15px] font-semibold'>
                    <img
                        src={primeUser?.image}
                        alt={primeUser?.name}
                        className='w-[50px] h-[50px] rounded-full p-1 border-[1px] border-white' />
                    <p>{primeUser?.name}</p>
                </div>
                <p className={`flex justify-center gap-x-1 py-1 font-bold rounded-[10px] mt-2`}>
                    <Clock className='text-yellow-400' />
