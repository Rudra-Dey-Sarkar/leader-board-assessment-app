...
import { CrownIcon, GiftIcon, TrophyIcon } from 'lucide-react';
...
function top3Leaders(activeFilter, users) {
    const result = [];
    if (activeFilter) {
        let temp = users.sort((a, b) => b.points.partyPoint.weeklyContributionPoint - a.points.partyPoint.weeklyContributionPoint).slice(0, 3);
        result.push(temp[1]);
        result.push(temp[0]);
        result.push(temp[2]);

    } else {
        let temp = users.sort((a, b) => b.points.partyPoint.weeklyCharmPoint - a.points.partyPoint.weeklyCharmPoint).slice(0, 3);
        result.push(temp[1]);
        result.push(temp[0]);
        result.push(temp[2]);
    }
...
function restLeaders(activeFilter, users) {
    ...
    return temp.sort((a, b) => activeFilter === true ? (b.points.partyPoint.weeklyContributionPoint - a.points.partyPoint.weeklyContributionPoint) : (b.points.partyPoint.weeklyCharmPoint - a.points.partyPoint.weeklyCharmPoint));
}

function PartyRanking({ users, primeUser }) {
            ...
            <div className='absolute top-0 p-2 z-10 w-full bg-white'>
                <div className='flex justify-center gap-2 m-2 p-1 bg-gray-200 rounded-full'>
                    ...
                    <button
                        ...
                        onClick={() => setActiveFilter(true)}>Weekly Contribution</button>
                    ...
                    <button
                        ...
                        onClick={() => setActiveFilter(false)}>Weekly Charm</button>
                </div>
            </div>

            {users?.length > 0 ?
                // Leader board
                <div className='w-full h-[95vh] bg-gradient-to-b from-[#dc5d32] to-[#f5c247] pt-20 pb-14 overflow-y-auto'>
                    <div className='flex justify-between items-center'>
                        <div />
                        <div />

                        {/* Countdown timer */}
                        <div className='flex gap-x-1 text-white bg-[#ff5c26] py-1 px-3 text-[12px] font-semibold text-center rounded-full'>
                            <p>Settlement Time</p>
                            <CountdownTimer />
                        </div>
                    ...
                    <img src="./party-ranking.png" alt="Wealth Rank Logo" className='top-0 left-[15%] w-[45%] h-fit m-auto' />
